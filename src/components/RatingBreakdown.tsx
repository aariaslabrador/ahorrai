...
function AmenityBadge({ label, pct }: { label: string; pct: number | null }) {
  if (pct === null) {
    return <span className="rounded-full bg-surface-2 px-2.5 py-1 text-xs text-muted">{label}: sin datos</span>;
  }
  const yes = pct >= 50;
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-xs font-medium ${
        yes ? "bg-emerald-50 text-emerald-700" : "bg-surface-2 text-muted"
      }`}
    >
      {label}: {yes ? "sí" : "no"} ({pct}%)
    </span>
...
export default function RatingBreakdown({ summary }: { summary: SupermarketRating | null | undefined }) {
  ...
  return (
    <div className="rounded-xl border border-line bg-surface p-4">
      <p className="text-sm font-semibold text-foreground">Por criterio</p>
